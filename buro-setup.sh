...
# Function to export private keys
export_private_key() {
    local key_type=$1
...
if ! vanacli wallet create --wallet.name default --wallet.coldkey default; then
    echo "Failed to create coldkey wallet."
    exit 1
fi

# Create wallet for hotkey
print_info "Creating a new Vana wallet (hotkey)..."
if ! vanacli wallet create --wallet.name default --wallet.hotkey default; then
    echo "Failed to create hotkey wallet."
    exit 1
fi
